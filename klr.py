...
#sparse (laurent) polynomials over Q
class laurent_polynomial():
    ...
    def __add__(self, other):
        if isinstance(other,int) or isinstance(other,Fraction):
            other_poly = laurent_polynomial({0:other})
        else:
            other_poly = other
...
class nilCoxeter():
    def __init__(self,a):
        '''
        returns a class representing the (Q) nilCoxeter algebra NC_a, the unital ring generated by u_1,...,u_a-1
        subject to u^2=0, braiding relation, and far commuting.
        '''
        pass
